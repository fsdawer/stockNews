# StockInsight — Claude Code Instructions

---

## ⛔ 절대 규칙: 구현 전 계획 수립 필수

모든 구현·수정·기능 요청 시 **반드시 계획을 먼저 수립하고 승인을 받은 후** 코드를 작성한다.  
복잡한 요청은 **ceo-agent를 먼저 호출**해 어떤 에이전트를 쓸지 조율한다.

**예외** (즉시 처리 가능): 단순 질문·코드 설명·오타·1줄 수정.  
이 규칙을 건너뛰는 것은 허용되지 않는다.

---

## 행동 원칙

1. **생각 먼저** — 불확실하면 가정 말고 물어본다. 여러 해석이 있으면 트레이드오프를 제시한다.
2. **최소한의 코드** — 요청된 것만 작성한다. 200줄이 50줄로 가능하면 다시 써라.
3. **외과적 수정** — 필요한 것만 건드린다. 내 변경이 만든 고아 코드만 정리한다.
4. **계획 먼저, 승인 후 실행** — 구현 전 텍스트 계획 작성 → 트레이드오프 제시 → 승인 후 실행.
5. **백엔드·프론트 동시 완성** — API 만들면 `frontend/src/` 화면까지 반드시 연결한다.
6. **진행 상황 실시간 중계** — 파일 하나 완성할 때마다 한 줄 알림. 사용자가 물어볼 필요 없어야 한다.
7. **⛔ 테스트 없이 완료 금지** — 기능 하나를 구현할 때마다 **반드시 직접 테스트**하고 결과를 보고한다. "완료했습니다"는 테스트 통과 후에만 말한다.

---

## 테스트 규칙 (건너뛰기 절대 금지)

### API 엔드포인트 변경 시
```bash
# 로컬에서 curl로 실제 응답 확인
curl -s "http://localhost:3000/api/[경로]?[파라미터]" | head -c 500
```
- 응답이 예상과 다르면 수정 후 재테스트
- "API 작성했습니다" → `curl`로 확인한 결과까지 보고

### UI 컴포넌트 변경 시
- `npm run build` PASS 확인
- 변경한 기능의 골든패스(정상 경로) 동작 확인
- 엣지케이스(로그인 안 됨, 데이터 없음 등) 확인

### 검색 기능
- 영어 티커 (AAPL, TSLA) 검색 → 결과 확인
- 한글 (애플, 테슬라, 코카콜라) 검색 → 영문 티커로 변환 확인
- 부분 입력 (테슬, 엔비) → 자동완성 확인

### 번역 기능
- `/api/news/translate` POST 호출 → `translated > 0` 확인
- Supabase에서 `headline_ko`가 한국어인지 확인

### 차트 기능
- 각 timespan (1분/5분/일봉/주봉/월봉) 탭 클릭 → 데이터 로드 확인
- 캔들스틱 차트 렌더링 확인

---

---

## 기능 구현 순서 (건너뛰기 금지)

```
1. [계획] ceo-agent → 요청 분석, 필요한 에이전트 선택
2. [설계] frontend-agent → UI/UX 흐름 설계 (화면 스케치 포함)
3. [설계] backend-agent → API 설계, DB 스키마, 트레이드오프 분석
4. [승인] 계획 취합 → 사용자 승인 획득
5. [분리] git worktree add ../stockinsight-<기능명> -b feature/<기능명>
6. [구현] backend-agent + frontend-agent 병렬 실행 → npm run build PASS
7. [검토] code-review 스킬 실행 → PASS
8. [머지] git merge --no-ff → worktree 삭제
```

---

## 종목 분석 워크플로우

사용자가 종목 분석을 요청하면 **반드시 아래 순서**를 따른다.

### 1단계: 3개 에이전트 병렬 실행

단일 `Agent` 도구 호출 메시지에서 아래 세 에이전트를 **동시에(병렬로)** 실행한다.

| 에이전트 | subagent_type | 역할 |
|---|---|---|
| 재무 분석가 | `financial-analyst-kr` | 매출 추이, 수익성 지표, 밸류에이션, 재무 건전성 |
| 뉴스 감성 분석가 | `news-sentiment-analyst` | 최근 1개월 뉴스 호재/악재 분류, 시장 심리 판정 |
| 업종 리서처 | `sector-researcher` | 글로벌 업종 흐름, 경쟁사 현황, 규제 변화, 업종 전망 |

세 에이전트는 **독립적이므로 순차 실행 금지** — 반드시 병렬로 띄운다.

```
# 올바른 호출 패턴 (한 메시지에 3개 Agent 호출)
Agent(subagent_type="financial-analyst-kr",  prompt="[종목명] 재무 분석")
Agent(subagent_type="news-sentiment-analyst", prompt="[종목명] 뉴스 감성 분석")
Agent(subagent_type="sector-researcher",      prompt="[종목명] 업종 분석")
```

### 2단계: 공격적 투자 전략가 종합

세 에이전트의 결과가 **모두** 반환된 후, `aggressive-investment-strategist` 에이전트를 실행하여 종합 투자 리포트를 작성한다.

프롬프트에 세 보고서 전문을 포함해야 한다:

```
다음은 [종목명]에 대한 세 분석가의 보고서입니다.

[재무 분석가 보고서]
{financial_analyst_result}

[뉴스 감성 분석가 보고서]
{news_sentiment_result}

[업종 리서처 보고서]
{sector_researcher_result}

위 세 보고서를 종합하여 최종 투자 판단을 내려주세요.
```

### 트리거 조건

아래 요청이 들어오면 이 워크플로우를 실행한다:

- "[종목명/티커] 분석해줘"
- "[종목명] 투자해도 될까?"
- "[종목명] 종합 분석 / 풀 분석"
- 그 외 종목에 대한 종합적 판단이 필요한 모든 요청

단순히 재무만, 뉴스만, 또는 업종만 요청하는 경우에는 해당 단일 에이전트만 실행한다.

## 에이전트 목록

```
.claude/agents/
├── financial-analyst-kr.md          # 재무 분석가
├── news-sentiment-analyst.md        # 뉴스 감성 분석가
├── sector-researcher.md             # 업종 리서처
└── aggressive-investment-strategist.md  # 공격적 투자 전략가 (최종 종합)
```

모든 에이전트는 `memory: project` 로 설정되어 있으므로 프로젝트 메모리를 공유한다.

---

## API / 외부 서비스 변경 규칙

### 절대 금지
- 사용자 승인 없이 API 제공자를 임의로 교체하지 않는다.
- 예: Finnhub → Yahoo Finance, Anthropic → Gemini 등

### 변경이 필요한 상황 발생 시 의무 절차

1. **문제 원인을 먼저 설명**한다.
   - 예: "Finnhub 무료 티어가 `/stock/candle` 엔드포인트에 403을 반환합니다."

2. **선택지를 제시**하고 사용자가 고르게 한다.
   - 예:
     > **A. Yahoo Finance** — API 키 불필요, 완전 무료, 비공식 API (언제든 막힐 수 있음)
     > **B. Alpha Vantage** — 무료 키 발급 필요, 하루 25회 제한
     > **C. FMP 유료 플랜 업그레이드** — 안정적이지만 비용 발생
     >
     > 어떤 방법으로 하시겠어요?

3. **사용자가 선택한 후에만** 코드를 변경한다.

### 현재 사용 중인 API 목록

| 서비스 | 용도 | 플랜 |
|--------|------|------|
| Finnhub WebSocket | 실시간 가격 | 무료 |
| Finnhub REST `/quote` | 전일 종가 | 무료 |
| Polygon.io | 90일 차트 히스토리 (기본값) | 무료 (5회/분) |
| FMP `/search-symbol` | 종목 검색 자동완성 | 무료 |
| Supabase | DB, Auth | 무료 |
| Gemini 1.5 Flash | 뉴스 번역/요약 | 무료 |

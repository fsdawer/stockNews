...
모든 구현·수정·기능 요청 시 **반드시 계획을 먼저 수립하고 승인을 받은 후** 코드를 작성한다.  
복잡한 요청은 **ceo-agent를 먼저 호출**해 어떤 에이전트를 쓸지 조율한다.

**예외** (즉시 처리 가능): 단순 질문·코드 설명·오타·1줄 수정.  
...
1. **생각 먼저** — 불확실하면 가정 말고 물어본다. 여러 해석이 있으면 트레이드오프를 제시한다.
2. **최소한의 코드** — 요청된 것만 작성한다. 200줄이 50줄로 가능하면 다시 써라.
3. **외과적 수정** — 필요한 것만 건드린다. 내 변경이 만든 고아 코드만 정리한다.
4. **계획 먼저, 승인 후 실행** — 구현 전 텍스트 계획 작성 → 트레이드오프 제시 → 승인 후 실행.
5. **백엔드·프론트 동시 완성** — API 만들면 `frontend/src/` 화면까지 반드시 연결한다.
6. **진행 상황 실시간 중계** — 파일 하나 완성할 때마다 한 줄 알림. 사용자가 물어볼 필요 없어야 한다.
...
```
1. [계획] ceo-agent → 요청 분석, 필요한 에이전트 선택
2. [설계] frontend-agent → UI/UX 흐름 설계 (화면 스케치 포함)
3. [설계] backend-agent → API 설계, DB 스키마, 트레이드오프 분석
4. [승인] 계획 취합 → 사용자 승인 획득
5. [분리] git worktree add ../stockinsight-<기능명> -b feature/<기능명>
6. [구현] backend-agent + frontend-agent 병렬 실행 → npm run build PASS
7. [검토] code-review 스킬 실행 → PASS
8. [머지] git merge --no-ff → worktree 삭제
```
